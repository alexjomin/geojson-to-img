...
class Render:
	...
	def get_tiles_for_bounds(self):
		...
		tile_x_range = range(x[0], x[1])
		tile_y_range = range(y[0], y[1])
		...
		print "%s x %s" % (self.number_of_rows , self.number_of_cols )
	...
	def generate_background(self, tiles) :
		...
		"""
		with Image(width=self.number_of_cols*256, height=self.number_of_rows*256) as img:
	...
	def get_xy(self, lon, lat) :
		...
		lat_rad = math.raidans(lat)
		n = math.pow(2.0, self.renderingZoom)

		tile_x = ((lon + 180) / 360) * n
		tile_y = (1 - (math.log(math.tan(lat_rad) + 1.0/math.cos(lat_rad)) / math.pi)) * n / 2.0

		x = math.round((tile_x - self.minxtile) * 256)
		y = math.round((tile_y - self.minytile) * 256)
	...
	# def generate_track() :
		...
		# track = new Imagick();
		# $track->newImage($this->numberOfCols*256, $this->numberOfRows*256, "none");

		# // Drawing Track Segments
		# for($i=0 ; $i+1 < count($this->wayPoints) ; $i++ ) {

		# 	$point1 = $this->wayPoints[$i] ;
		# 	$point2 = $this->wayPoints[$i+1] ;

		# 	$array = $this->getPixelPosForCoordinates($point1->latitude, $point1->longitude) ;
		# 	$x1 = $array[0] ;
		# 	$y1 = $array[1] ;

		# 	$array = $this->getPixelPosForCoordinates($point2->latitude, $point2->longitude) ;
		# 	$x2 = $array[0] ;
		# 	$y2 = $array[1] ;
		...
		# 	$track->drawImage( $draw );

		# }
		# // Adding the track to the background
		# $this->img->compositeImage($track, Imagick::COMPOSITE_DEFAULT, 0,0);
